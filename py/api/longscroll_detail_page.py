...
def call_synvow_llm(model, system_prompt, user_prompt, image_urls=None, temperature=0.3, seed=None, max_tokens=4096):
    ...
    for attempt in range(3):
        try:
            ...
            content = synvow_auth.parse_chat_response(res.json()) or ""
            if not content.strip():
                raise RuntimeError("SynVow 返回成功，但未解析到文本内容")
            return content.strip()
        except Exception as exc:
            last_error = exc
            print(f"[SynVowLongScroll] LLM request failed ({attempt + 1}/3): {exc}")
            if attempt < 2:
                time.sleep(5 * (attempt + 1))
    raise RuntimeError(
        "SynVow 接口请求失败或远端断开。已按 SynVow 登录态方式重试 3 次。"
        f"最后错误：{last_error}"
    )
...
def collect_uploaded_image_urls(product_images, reference_images):
    ...
    for index, image in enumerate(product_images, start=1):
        if image is None:
            continue
        try:
            image_urls.append(_upload_image(api_key, image))
        except Exception as exc:
            raise RuntimeError(f"产品图_{index} 上传失败：{exc}") from exc
    for index, image in enumerate(reference_images, start=1):
        ...
        try:
            ...
            print(f"[SynVowLongScroll] 参考图_{index} 上传失败，已跳过：{exc}")
    return image_urls
...
class SynVowLongScrollNarrativePlanner:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                "产品图_1": ("IMAGE",),
                "模型": (GEMINI_MODEL_OPTIONS, {"default": DEFAULT_MODEL}),
...
class SynVowLongScrollPageStructurePlanner:
    ...
    @classmethod
    def IS_CHANGED(cls, 叙事结构_JSON, 长卷视觉母版说明, 模型, 结构修正要求_可选="", temperature=0.25, 种子=0):
        return stable_text_fingerprint(
            叙事结构_JSON,
            长卷视觉母版说明,
            模型,
    ...
    def plan_structure(self, 叙事结构_JSON, 长卷视觉母版说明, 模型, 结构修正要求_可选="", temperature=0.25, 种子=0):
        narrative = load_json(叙事结构_JSON, {})
        if not isinstance(narrative, dict):
            narrative = {}
        sections = narrative.get("sections", [])
        if not sections:
            raise RuntimeError("未找到叙事结构 sections，请先连接 GPT-image2详情页规划。")
        ...
        def request_chunk(start_id, end_id):
            ...
            content = call_synvow_llm(模型, system_prompt, user_prompt, None, temperature, 种子)
...
class SynVowLongScrollPromptBatchBuilder:
    ...
    @classmethod
    def IS_CHANGED(cls, 页面结构蓝图_JSON, 长卷视觉母版说明, 模型, 叙事结构_JSON="", 出图提示词修正_可选="", temperature=0.25, 种子=0):
        return stable_text_fingerprint(
            页面结构蓝图_JSON,
            长卷视觉母版说明,
            模型,
    ...
    def build(self, 页面结构蓝图_JSON, 长卷视觉母版说明, 模型, 叙事结构_JSON="", 出图提示词修正_可选="", temperature=0.25, 种子=0):
        ...
        if use_page_structure:
            ...
            if not page_structure["items"]:
                raise RuntimeError("页面结构蓝图 items 为空，请先重新运行 GPT-image2详情页结构。")
            ...
            module_ratio_plan = build_module_ratio_plan(target_slice_count)
            raise RuntimeError("未连接页面结构蓝图。已关闭从叙事结构自动补齐页面结构，请先运行 GPT-image2详情页结构。")
        else:
            raise RuntimeError("未找到页面结构蓝图 items，也未找到叙事结构 sections。请先连接 GPT-image2详情页结构。")
...
NODE_CLASS_MAPPINGS = {
    "SynVowLongScrollNarrativePlanner": SynVowLongScrollNarrativePlanner,
    "SynVowLongScrollPageStructurePlanner": SynVowLongScrollPageStructurePlanner,
    "SynVowLongScrollPromptBatchBuilder": SynVowLongScrollPromptBatchBuilder,
    "SynVowLongScrollImageListConcat": SynVowLongScrollImageListConcat,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "SynVowLongScrollNarrativePlanner": "GPT-image2详情页规划",
    "SynVowLongScrollPageStructurePlanner": "GPT-image2详情页结构",
    "SynVowLongScrollPromptBatchBuilder": "GPT-image2详情页批量提示词",
    "SynVowLongScrollImageListConcat": "详情页图像列表顺序拼接长图",
}
