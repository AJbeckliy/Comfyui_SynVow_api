...
from .video_common import (
    ...
    upload_images, upload_video_file,
)

SEEDANCE2_SUBMIT_URL = f"{DIRECT_API_BASE}/api/models/image/edit"
SEEDANCE2_POLL_URL = f"{DIRECT_API_BASE}/api/models/tasks"

MODEL_OPTIONS = ["seedance2.0", "seedance2.0-fast"]
...
def _build_files(image_urls=None, video_url=None, audio_url=None):
    files = []
    if image_urls:
        for u in image_urls:
            files.append({"url": u, "type": "image"})
    if video_url and video_url.strip():
        files.append({"url": video_url.strip(), "type": "video"})
    if audio_url and audio_url.strip():
        files.append({"url": audio_url.strip(), "type": "audio"})
...
def _submit_task(api_key, prompt, model, ratio, duration, resolution, files=None):
    ...
    for attempt in range(3):
        try:
            ...
            print(f"[Seedance2] 提交成功: task_id={task_id} model={model}")
            return task_id, consumption_id
        except RuntimeError:
            raise
        except Exception as e:
            print(f"[Seedance2] 提交重试 {attempt+1}/3: {e}")
            if attempt < 2:
                time.sleep(5)
            else:
                raise
    ...
    while True:
        ...
        try:
            ...
            if res.status_code in (429, 500, 503):
                print(f"[Seedance2] 轮询 {count}: HTTP {res.status_code}, 退避10秒")
                time.sleep(10)
                continue
            data = res.json() if res.status_code == 200 else {}
            inner = data.get("data") or data
            status = str(inner.get("status") or inner.get("task_status") or "").upper()
            print(f"⏳ [Seedance2][{task_id[:8]}] 轮询 {count}: {status or '(无状态)'} | raw={str(data)[:400]}")
...
class SynVowSeedance2Video:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "模型": (MODEL_OPTIONS, {"default": "seedance2.0-fast"}),
            ...
            "optional": {
                ...
                "audio_url": ("STRING", {"forceInput": True}),
    ...
    def generate_video(self, prompt, 模型, ratio, duration, resolution,
                       ...
                       video_url="", audio_url="", filename="", save_path=""):
        try:
        ...
        files = _build_files(image_urls=image_urls, video_url=video_url, audio_url=audio_url)

        try:
            task_id, consumption_id = _submit_task(api_key, prompt, 模型, ratio, duration, resolution, files)
            result = _poll_seedance2(api_key, task_id, 模型, consumption_id=consumption_id)
            url = _extract_video_url(result)
            if not url:
            ...
            info = json.dumps({
                ...
                "model": 模型, "video_url": url, "video_path": path,
            }, ensure_ascii=False)
...
class SynVowSeedance2VideoBatch:
    ...
    def process_batch(self, prompts_list, 模型, ratio, duration, resolution, 批次数量,
                      image_1=None, image_2=None, image_3=None, image_4=None,
                      image_5=None, image_6=None, image_7=None, image_8=None,
                      video_url=None, audio_url=None, filename=None, save_path=None):
        _u = lambda v, d=None: v[0] if isinstance(v, list) and v else (v if v is not None else d)
        model = _u(模型, "seedance2.0-fast")
        ...
        _vu = _u(video_url, "")
        _au = _u(audio_url, "")
        files = _build_files(image_urls=image_urls, video_url=_vu, audio_url=_au)
