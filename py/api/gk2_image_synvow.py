...
def _build_body(prompt, aspect_ratio, resolution, image_urls):
    body = {
        "model": _MODEL,
        "prompt": prompt or "",
        "aspect_ratio": aspect_ratio if aspect_ratio in _ASPECT_RATIOS else _DEFAULT_RATIO,
        "resolution": resolution if resolution in _RESOLUTIONS else _DEFAULT_RESOLUTION,
    }
...
def _run_tasks(tasks, aspect_ratio, resolution, api_key):
    total = len(tasks)
    ...
    for i, (prompt, imgs) in enumerate(tasks):
        try:
            ...
            body = _build_body(prompt, aspect_ratio, resolution, urls)
...
class SynVowGkImage20:
    FUNCTION = "generate"
    CATEGORY = "💫SynVow_api/api/图像"
    INPUT_IS_LIST = True
    DESCRIPTION = "SynVow GK2.0 文生图/图生图"
    ...
    def generate(self, model_type=None, aspect_ratio=None, resolution=None, seed=None,
        ...
        imgs = _collect_images(image1, image2, image3)
        api_key = synvow_auth.read_api_key()
        p = str(prompt).strip() if prompt else ""
        image_urls = _run_tasks([(p, imgs)], aspect_ratio, resolution, api_key)
...
class SynVowGkImage20_TBatch:
    FUNCTION = "process_batch"
    CATEGORY = "💫SynVow_api/api/图像"
    INPUT_IS_LIST = True
    OUTPUT_IS_LIST = (True, False)
    DESCRIPTION = "SynVow GK2.0 提示词批量"
    ...
    def process_batch(self, model_type=None, aspect_ratio=None, resolution=None, seed=None,
                      prompts_list=None, image1=None, image2=None, image3=None):
        del model_type, seed
        aspect_ratio = _unpack(aspect_ratio) or _DEFAULT_RATIO
        resolution = _unpack(resolution) or _DEFAULT_RESOLUTION
        imgs = _collect_images(image1, image2, image3)
        ...
        image_urls = _run_tasks(tasks, aspect_ratio, resolution, api_key)
...
class SynVowGkImage20_IBatch:
    FUNCTION = "process_batch"
    CATEGORY = "💫SynVow_api/api/图像"
    INPUT_IS_LIST = True
    OUTPUT_IS_LIST = (True, False)
    DESCRIPTION = "SynVow GK2.0 图像批量"
    ...
    def process_batch(self, images_list1, model_type=None, aspect_ratio=None, resolution=None,
                      prompt=None, seed=None, prompts_list=None):
        del model_type, seed
        aspect_ratio = _unpack(aspect_ratio) or _DEFAULT_RATIO
        resolution = _unpack(resolution) or _DEFAULT_RESOLUTION
        prompt = _unpack(prompt)
        ...
        if not images:
            raise ValueError("GK2.0 图像批量：需要接入图像组1")
...
class SynVowGkImage20_TIBatch:
    FUNCTION = "process_batch"
    CATEGORY = "💫SynVow_api/api/图像"
    INPUT_IS_LIST = True
    OUTPUT_IS_LIST = (True, False)
    DESCRIPTION = "SynVow GK2.0 双批量"
    ...
    def process_batch(self, images_list1, model_type=None, aspect_ratio=None, resolution=None,
                      prompt_order=None, seed=None, prompts_list=None):
        del model_type, seed
        aspect_ratio = _unpack(aspect_ratio) or _DEFAULT_RATIO
        resolution = _unpack(resolution) or _DEFAULT_RESOLUTION
        ...
        images = _list_tensors(images_list1)
        if not images:
            raise ValueError("GK2.0 双批量：需要接入图像组1")
        prompts = _normalize_prompts(prompts_list)
...
NODE_CLASS_MAPPINGS = {
    "SynVowGkImage20": SynVowGkImage20,
    "SynVowGkImage20_TBatch": SynVowGkImage20_TBatch,
    "SynVowGkImage20_IBatch": SynVowGkImage20_IBatch,
    "SynVowGkImage20_TIBatch": SynVowGkImage20_TIBatch,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "SynVowGkImage20": "SynVow GK2.0",
    "SynVowGkImage20_TBatch": "SynVow GK2.0 (T_batch)",
    "SynVowGkImage20_IBatch": "SynVow GK2.0 (I_batch)",
    "SynVowGkImage20_TIBatch": "SynVow GK2.0 (T_I_batch)",
}
